...
user_photo = []
user_photo_name = []
FACE_DETECTOR_PATH = "{base_path}/haarcascade_frontalface_default.xml".format(base_path=os.path.abspath(os.path.dirname(__file__)))
...
def digital_makeup(request):
    ...
    # pil_image.show()
    cv2.imwrite('./assets/img/digital_makeup.png', np.asarray(pil_image))
    return HttpResponse('/assets/img/digital_makeup.png')
...
def find_facial_feature(request):
    ...
    # Show the picture
    # pil_image.show()
    cv2.imwrite('./assets/img/find_facial_feature.png', np.asarray(pil_image))
    return HttpResponse('/assets/img/find_facial_feature.png')
...
def find_face(request):
    ...
    cv2.imwrite('./assets/img/find_face.png', np.asarray(pil_image))
    return HttpResponse('/assets/img/find_face.png')
...
def face_recog(request):
    ...
    del draw
    # Display the resulting image
    cv2.imwrite('./assets/img/face_reco.png', np.asarray(pil_image))
    return HttpResponse('/assets/img/face_reco.png')
...
class EmployeeImage(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            obj = form.save()
            print(obj.fields['emp_image'].name)
            ...
            if kindtype == 'image':
                ...
                return render(request,'emp_image.html',{'data':checkphotoname, 'user': request.session['user'],'url': f"{Target_DIR}/{request.FILES['emp_image']}"})
            elif kindtype == 'video':
                return render(request,'emp_image.html',{'video_url': f"{Target_DIR}/{request.FILES['emp_image']}", 'user': request.session['user']})
